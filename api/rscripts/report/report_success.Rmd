...
```{r libraries, echo = FALSE, results = 'asis', error = TRUE}
# define libraries
...
demographics <- data.frame(ID = params$user_data$id,
...
# map
map_target <- params$map_pk$map$display
map_pk <- params$map_pk$data
...
if (nrow(genetics) > 0) {
  kable(genetics, format = "latex", booktabs = TRUE, escape = FALSE,
...
```{r clindata-tab, echo = F, results = 'asis', error = T}
###################
if (nrow(meas_conc) > 0) {
  glue("The following table displays the concentration measurements of {compound} provided to the system. The clinical data entered into the system are used to estimate individual model parameters using Maximum a Posteriori (MAP) estimation to predict patient specific plasma concentration-time profiles of {compound}. \n")
  # add unit to conc column name
  unit_clinconc <- params$user_data$clinical_conc$value_unit
  colnames(meas_conc)[colnames(meas_conc) == "Concentration"] <- paste0("Concentration [", unit_clinconc, "]")
  kable(meas_conc, format = "latex", booktabs = TRUE, escape = FALSE,
        caption = glue("Clinical measurements of {compound} for patient {patient}"),
        label = "ClinData") |>
    kable_styling(latex_options = c("striped", "HOLD_position"), font_size = 12) |>
    row_spec(0, bold = TRUE)
} else {
  glue("No clinical data for {compound} have been provided")
}
```

# Results
```{r map, echo = F, results = 'asis', error = T}
# if no measured clinical data no map can be performed
# check for clinical data
if (nrow(meas_conc) > 0) {
  cat("## MAP estimation \n")
  print(glue("Based on the clinical data provided, a MAP approach was applied to calculate individual {compound} pharmacokinetics of patient {patient} by adjusting {map_target}. Results of the MAP are displayed in the following Figure and Table. \n\n"))

  # add column date_time
  meas_conc <- meas_conc |>
    mutate(date_time = paste(Date, `Clock time`))

  # transform column date_time to date
  meas_conc$date_time <- as.POSIXct(meas_conc$date_time, format = "%Y-%m-%d %H:%M:%S")

  # extract compartment (PK-Sim path)
  cmt <- unique(map_pk$CMT)
  parts <- strsplit(cmt, "\\|")[[1]]

  # extract compound/compartment name from PK-Sim compartment path
  drug_name <- parts[3]
  sample_info <- parts[4] 

  # get unit
  conc_unit <- params$map_pk$metaData$PRED$unit

  # plot pop mean, individual profile and measured clinical data
  map_plot <- ggplot() +
    geom_line(data = map_pk, aes(x = date_time, y = PRED, color = "Population prediction")) +
    geom_line(data = map_pk, aes(x = date_time, y = IPRED, color = "Individual prediction")) +
    geom_point(data = meas_conc, aes(x = date_time, y = `Concentration [mg/L]`, color = "Conc Measurement")) +
    labs(
    ...
    scale_color_manual(values = c("Population prediction" = "black",
                                  "Individual prediction" = "red",
                                  "Conc Measurement" = "red")) +
    theme_classic() +
    theme(legend.position = "bottom")

  print(map_plot)

  # extract pop value and individual value of MAP adjusted parameter
  map_info <- params$map_pk$map |>
    select(display, pop_values, ind_values) |>
    rename(Parameter = display,
           `Population value` = pop_values,
           `Individually estimated value` = ind_values)

  # print MAP results in table
  kable(map_info, format = "latex", booktabs = TRUE, escape = FALSE,
        caption = glue("Individualized {map_target} using the MAP approach")) |>
...
if (nrow(meas_conc) > 0) {
  dose_adapt <- dose_adapt +
    geom_point(data = meas_conc, aes(x = date_time, y = `Concentration [mg/L]`),
               color = "darkred",
               show.legend = FALSE)
  print(dose_adapt)
} else {
...
pk_data <- params$dose_pk$summary |>
  ...
  rename(`Steady state` = in_steady_state,
         AUC = auc,
         `Dose` = label,
...
kable(pk_data, format = "latex", booktabs = TRUE, escape = FALSE,
...
for (i in seq_len(length(split_result))) {
  ...
  sub_plots <- ggplot(data = dose_set, aes(x = date_time, y = DPRED)) +
    ...
    annotate("rect", xmin = dose_adapt_start_date, xmax = max(dose_set$date_time),
    ...
    geom_line(data = initial_dose, aes(x = date_time, y = DPRED, color = init_label),
              linetype = "dashed") +
